...
# Default profile avatars — assigned randomly to new users on signup.
# Files live under /app/backend/static/avatars and are served at
# /api/static/avatars/<name>.png via the StaticFiles mount below.
...
def pick_random_default_avatar() -> str:
    """Return a server-relative URL for a random default avatar."""
    return f"/api/static/avatars/{random.choice(DEFAULT_AVATAR_FILES)}"
...
async def register(user_data: UserRegister):
    ...
    user_doc = {
        ...
        "photoUrl": pick_random_default_avatar(),
        "bio": "",
...
@app.on_event("startup")
async def _backfill_default_avatars():
    """Give every existing user with no (or stale-default) photoUrl a random default avatar."""
    try:
        valid_urls = {f"/api/static/avatars/{f}" for f in DEFAULT_AVATAR_FILES}
        # Stale defaults from a previous run (older filenames) start with this prefix
        # but are not in the current valid set.
        cursor = db.users.find(
            {
                "$or": [
                    ...
                    # stale autogenerated defaults: prefix matches but URL is no longer
                    # in the current file set
                    {"photoUrl": {"$regex": r"^/api/static/avatars/default-"}},
                ]
            },
            {"_id": 1, "photoUrl": 1},
        )
        ...
        rewritten = 0
        for u in users:
            current = u.get("photoUrl")
            if current and current in valid_urls:
                continue  # already valid, skip
            await db.users.update_one(
                {"_id": u["_id"]},
                {"$set": {"photoUrl": pick_random_default_avatar()}},
            )
            rewritten += 1
        if rewritten:
